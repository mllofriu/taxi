...
combinations <- expand.grid(method=c('ql', 'msql'), trial = seq(1,numTrials))

rte <- foreach (combination=1:nrow(combinations),.verbose=T, .packages=c('foreach','sp','rgeos','plotrix','plyr', 'ggplot2'), .combine=rbind, .export=c(as.vector(lsf.str()))) %dopar%{
  trial <- combinations[combination,'trial']
  method <- combinations[combination, 'method']
  # Init ql value
  if (method == 'msac')
